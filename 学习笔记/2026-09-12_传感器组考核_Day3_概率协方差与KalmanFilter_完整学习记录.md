...
$$ \omega_m= \begin{bmatrix} \omega_x\\ \omega_y\\ \omega_z \end{bmatrix} $$

单位是 $rad/s$。它**不是直接输出姿态**；姿态需要通过 $\text{Angular Velocity}\rightarrow Integration \rightarrow Attitude$ 得到。
...
$$ \delta \theta(t) \approx \int_0^t \delta \omega(\tau)d\tau $$

会逐渐累积成 Attitude Error，最终形成 Drift（漂移）。核心因果链：

$$ \boxed{\text{Gyro Error}\rightarrow Integration \rightarrow\text{Attitude Error Accumulation}\rightarrow Drift } $$

## 2.3 增量旋转向量 $\Delta\theta$ 的物理意义

$$ \Delta\theta = (\omega_m-\hat b_g)\Delta t $$
...
$$ \boxed{\text{State Estimation}=\text{State Estimate}+\text{State Uncertainty}} $$
...
$$ \boxed{ \text{真实状态可能位于哪些值，以及这些值分别有多可能} } $$
...
$$ \boxed{ Mean = \text{Probability Distribution}\ \text{的中心位置} } $$
...
Variance 记为 $\sigma^2$，Standard Deviation（标准差）记为 $\sigma$。例如 $\sigma=2^\circ$ 时 $\sigma^2=4\text{ deg}^2$。姿态滤波中一般采用弧度，因此姿态 Variance 常见单位是 $rad^2$。
...
$$ \boxed{ Gaussian = Mean + Variance } $$

例如 $Yaw\sim\mathcal N(10^\circ,0.01\text{ deg}^2)$ 表示当前最可能在 $10^\circ$ 附近且 uncertainty 很小；而 $Yaw\sim\mathcal N(10^\circ,25\text{ deg}^2)$ 虽然中心仍是 $10^\circ$，uncertainty 大得多。
...
$$ \boxed{ \text{两个变量相对各自} Mean \text{的偏差是否具有一起变化的趋势} } $$
...
$$ P= \begin{bmatrix} \sigma_p^2 & Cov(p,v)\\ Cov(v,p) & \sigma_v^2 \end{bmatrix} $$

核心结构是：

$$ \begin{aligned} &\boxed{\text{Diagonal Terms}= \text{每个} State \text{自己的} Variance }\\ &\boxed{ Off\text{-}\text{Diagonal Terms} = \text{不同} State Error \text{之间的} Covariance } \end{aligned} $$

例如

$$ P= \begin{bmatrix} 4&1.5\\ 1.5&9 \end{bmatrix} $$
...
$$ \boxed{ \text{我们对} Error \text{的统计不确定性} } $$

因此 $P_{pp}=100$ 不能理解为“当前 position 一定错了 100 m”。如果 $P_{pp}=\text{100 m}^2$，它表示的是 Position Error Variance，对应 Standard Deviation：
...
$$ \boxed{ P = \text{当前} State Estimate \text{的} uncertainty } $$
...
$$ \boxed{ Q =\text{Process Noise Covariance}} $$

含义是：

$$ \boxed{ Q = \text{这一次} Prediction \text{过程中新增的} uncertainty } $$
...
$$ \boxed{ R =\text{Measurement Noise Covariance}} $$

含义是：

$$ \boxed{ R = \text{这一次} Observation \text{自身的} uncertainty } $$

如果 FAST-LIO 某一帧 covariance 很大，则 $R\uparrow$，意味着这一帧 Observation 应该更少相信。

## 4.4 P / Q / R 的最终区分

$$ \begin{aligned} &\boxed{ P=\text{我现在有多不确定} }\\ &\boxed{ Q=\text{我预测这一步又增加多少不确定性} }\\ &\boxed{ R=\text{外部观测本身有多不确定} } \end{aligned} $$
...
$$ \boxed{ x_k = F_kx_{k-1} + B_ku_k + w_k } $$

Observation Model 把 State 映射到 Sensor 能观测的空间：

$$ \boxed{ z_k = H_kx_k + v_k } $$
...
$$ \boxed{ \text{上一时刻} State \text{怎样传播到下一时刻} State } $$

以 $x=\begin{bmatrix} p\\ v \end{bmatrix}$ 的恒速模型为例，$p_k=p_{k-1}+v_{k-1}\Delta t$ 且 $v_k=v_{k-1}$，对应：

$$ \boxed{ F= \begin{bmatrix} 1&\Delta t\\ 0&1 \end{bmatrix} } $$
...
$$ \boxed{ F =\text{State propagation relationship}} $$

## 5.3 H：Observation Matrix

$H$ 的核心作用是：

$$ \boxed{ \text{把完整} State \text{映射到} Sensor \text{能观测的空间} } $$
...
$$ \boxed{ H\in\mathbb R^{m\times n} } $$

也就是：

$$ \boxed{ H:\text{State Space}\rightarrow\text{Measurement Space}} $$
...
$$ \begin{aligned} &\boxed{ F: State\ \text{怎样随时间传播} }\\ &\boxed{ H: Sensor\ \text{能从} State \text{中看到什么} } \end{aligned} $$

即 $x_{k-1} \xrightarrow{F} x_k$，而 $x_k \xrightarrow{H}\text{Predicted Measurement}$。
...
$$ \boxed{ \hat x_{k-1}^+ \rightarrow Prediction \rightarrow \hat x_k^- \rightarrow Correction \rightarrow \hat x_k^+ } $$

Covariance 同理：

$$ P_{k-1}^+ \rightarrow P_k^- \rightarrow P_k^+ $$

其中 $P_k^-$ 表示第 $k$ 帧 Observation 融合之前的状态 uncertainty。

## 6.1 Prediction —— State

$$ \boxed{ \hat x_k^- = F_k\hat x_{k-1}^+ + B_ku_k } $$
...
$$ \hat x_k^-= \begin{bmatrix} 11\\ 2 \end{bmatrix} $$
...
$$ \boxed{ FPF^T = \text{旧} uncertainty \text{根据} State propagation \text{关系进行传播} } $$
...
$$ \boxed{ \text{本次} Prediction \text{新加入的} Process uncertainty } $$

最终：

$$ \boxed{ P_k^- = \text{旧} uncertainty \text{的传播} + \text{新的} Process uncertainty } $$
...
$$ \boxed{ Residual / Innovation \text{自身的不确定性} } $$

其中 $HP^-H^T$ 表示 Prediction uncertainty 映射到 Measurement Space 后的 uncertainty，而 $R$ 表示 Measurement 自己的 uncertainty。所以：

$$ \boxed{ S =\text{Prediction uncertainty in measurement space}+\text{Measurement uncertainty}} $$
...
$$ \boxed{ \text{更相信} Measurement } $$

如果 $P^-\ll R$，说明 Prediction 很可靠、Measurement 很不可靠，则 $K\rightarrow0$，所以：

$$ \boxed{ \text{更相信} Prediction } $$

因此：

$$ \boxed{\text{Kalman Gain}= \text{根据} Prediction uncertainty \text{和} Measurement uncertainty \text{自动计算的动态融合权重} } $$

它不是人为固定设置的常数。

## 8.2 State Update：状态更新

$$ \boxed{ \hat x^+ = \hat x^-+Kr } $$
...
$$ \boxed{ \text{从} Prediction \text{出发，沿着} Observation \text{指出的方向修正一部分} } $$
...
$$ \boxed{ Residual \text{只描述} Prediction \text{与} Measurement \text{差多少} } $$

而：

$$ \boxed{ \text{这个差应该用于修正多少，由} K \text{决定} } $$
...
$$ \boxed{ \text{一维} Kalman Filter \text{在多维相互关联} State \text{上的推广} } $$
...
$$ \begin{aligned} &\boxed{ IMU \rightarrow Prediction }\\ &\boxed{ FAST\text{-}LIO \rightarrow Observation } \end{aligned} $$
...
$$ \boxed{ R_\theta = diag( cov_{33}, cov_{44}, cov_{55} ) } $$

位置选做：

$$ \boxed{ R_p = diag( cov_{00}, cov_{11}, cov_{22} ) } $$
...
$$ \boxed{\text{Kalman Filter}\text{不是只维护} State Estimate\text{，} \text{还同时维护} State Estimate \text{的} Uncertainty } $$

每一帧有两条并行主线。State Estimate 主线：

$$ \hat x_{k-1}^+ \rightarrow \hat x_k^- \rightarrow \hat x_k^+ $$
...
$$ \boxed{ \text{传播} State + \text{传播} / \text{增加} Uncertainty } $$

Observation 也携带两类信息：

$$ \boxed{ Measurement +\text{Measurement Uncertainty}} $$
...
$$ \boxed{ Error \text{的统计} uncertainty } $$
...
$$ \begin{aligned} &Variance=\sigma^2\\ &\text{Standard Deviation}=\sigma \end{aligned} $$

Covariance 与 Correlation Coefficient 不同：Covariance 能反映两个变量误差之间的联合变化关系，但不能直接和标准化后的 Correlation Coefficient（相关系数）等同。

P ≠ Q ≠ R：

$$ \begin{aligned} &P: \text{当前} State Estimate uncertainty\\ &Q: Prediction \text{新增} process uncertainty\\ &R: Observation uncertainty \end{aligned} $$

F ≠ H：

$$ \begin{aligned} &F:\text{State propagation}\\ &H: State\rightarrow Measurement \end{aligned} $$
...
$$ \boxed{ Prediction \text{和} Observation \text{怎样根据} uncertainty \text{进行融合} } $$

Day 3 学习的是：

$$ \boxed{\text{Linear Kalman Filter}} $$

但题目二真正的姿态状态包含 Quaternion，Quaternion Rotation 是 Nonlinear（非线性）的，而且：

$$ Quaternion: \text{4 parameters} $$
...
$$ \boxed{\text{Quaternion Prediction}+\text{Error State}+ EKF + FAST\text{-}LIO\text{ Attitude Observation}} $$

得到：

$$ \boxed{ \text{6D Attitude}\ Error\text{-}\text{State EKF} } $$

最终一句话总结：

$$ \boxed{ Day\ 1\text{：姿态怎么表示} \rightarrow Day\ 2\text{：}IMU \text{怎么预测姿态} \rightarrow Day\ 3\text{：}Prediction \text{和} Observation \text{怎么依据} Uncertainty \text{融合} } $$

Day 4 才会进一步解决：

$$ \boxed{ \text{如何把} Kalman Filter \text{的思想真正落到非线性的} Quaternion \text{姿态估计上} } $$
