"""Run the complete Phase 2.4A 6D ESKF and write result diagnostics."""
...
from runner6d import run_eskf6d, save_eskf6d_run
...
def run(processed_directory: Path, output_directory: Path) -> dict[str, object]:
    ...
    eskf = ESKF6D.from_initialization(
        initialization, DEFAULT_CONFIG.numerical
    )
    run_result = run_eskf6d(
        imu,
        pose,
        init_stats,
        alignment,
    ...
    paths = save_eskf6d_run(run_result, output_directory)
